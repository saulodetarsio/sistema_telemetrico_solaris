...
def definir_valores_da_linha(linha):
    ...
    localizacao = "["+div[5]+","+lng+"]"


    print(medidas, localizacao)

    gravar_linha_csv(div[0], div[1], div[2], div[3], 
        round(float(div[4])/10, 2), float(div[5])/1000000, float(lng)/1000000)
...
try:
    ...
    with open('dados_iffsolaris_provas78_dsb2020.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["bateria_principal", "baterias_auxiliares", "placas", "corrente_motor", "velocidade", "latitude", "longitude"])


    ref_arquivo = open("dados_iffsolaris_prova3_dsb2020.txt","r")

    client = mqtt.Client('publicador_{}'.format(10)) # instancia a conexao
